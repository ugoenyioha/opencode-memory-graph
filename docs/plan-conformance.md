...
| `p4`  | Plugin lifecycle integration and context hooks                       | Partial                   | Active hooks in `src/index.ts` (`experimental.chat.system.transform`, `chat.message`, `experimental.session.compacting`, `tool.execute.after` TODO); full target hook model in `docs/design.md` not fully wired                                                                         |
